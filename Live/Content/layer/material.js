        ...
        material = {
            open: function (opt) {
                ...
                var dft = {//默认参数
                    ...
                    doc: "",                      //doc：字符串类型behavior的执行操作，默认是链接类型，执行window.open(‘链接地址’)，如果是script，需写javascript:alert(123)
                ...
                ops = $.extend(dft, opt),
                htmls = {
                    ovl: "<div class='Popup-mask' id='mask-" + idc + "'></div><div class='Popup-material' id='ovl-" + idc + "'><div class='Material-content'><video class='materimg'  src=''  controls='' style='width:100%'/></video></div></div>",
                ...
                material.prototype = {
                    ...
                    setdata: function () {
                        var f = function (obj) {
                                obj.css({
                                    top: '0px',
                                    left: (viewWidth() - obj.outerWidth()) / 2
                                })
                                obj.addClass('x');
                            },
                            j = function (obj) {
                                obj.css({
                                    top: (viewHeight() - obj.outerHeight()) / 2,
                                    right: '0px'
                                })
                                obj.addClass('y');
                            }
                        ...
                        if (ops.position == 'center') {
                            var c = function (obj) {
                             ...
                             get("materimg").error(function () {
                                 c(get('ovlid'));
                                 layer.alert("src: 图片加载失败!");
                             })
                            this.windowResiz('xy');
                            ...
                            for (var key in ops.style) {
                                switch (key) {
                                    case 'content':
                                        ...
                                        break;
                                    default: layer.alert("style: css " + key + "参数未定义");
                                }
                            }
                        } else {
                            layer.alert("style: 参数未定义,或'style'字符错误!")
                        }
                    },
                    bindEvent: function () {
                        var _this = this,
                    ...
                    openlink: function () {
                        if (ops.doc) {
                            ...
                            console.log('doc: 链接地址未填写,或doc字符错误!');
                        }
                    },
                    scriptdo: function () {
                        ...
                        get("materimg").on('click', function () {
                            var flage = true;
                            if (flage){console.log("material.js执行" + ops.doc + "");}
                            if (flage){evil(ops.doc); console.log("执行" + ops.doc + "正常");}
                        })
                    },
                    hide: function () {
                        var ovl = get("ovlid");
